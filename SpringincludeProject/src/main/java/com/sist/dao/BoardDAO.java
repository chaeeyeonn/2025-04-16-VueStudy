...
public class BoardDAO {
	...
	// 메모리 할당하면 싱글톤 패턴 걸면 안 됨 => 스프링 자체가 싱글톤 자제 생성
	...
	public void boardInsert(BoardVO vo)
	{
		try
		{
			...
			String sql="INSERT INTO springReplyBoard(no,name,subject,content,pwd,group_id) "
					+ "VALUES(srb_no_seq.nextval,?,?,?,?,(SELECT NVL(MAX(group_id)+1,1) "
	...
	public BoardVO boardDetailData(int no)
	{
		...
		try
		{
			...
			String sql="UPDATE springReplyBoard SET hit=hit+1 "
					+ "WHERE no="+no;
			ps=conn.prepareStatement(sql);
			ps.executeUpdate();
	...
	public boolean boardUpdate(BoardVO vo)
	{
		...
		try
		{
			...
			{
				bCheck=true;
				sql="UPDATE springReplyBoard SET name=?,subject=?,content=? WHERE no=?";
				ps=conn.prepareStatement(sql);
				ps.setString(1, vo.getName());
				ps.setString(2, vo.getSubject());
				ps.setString(3, vo.getContent());
	...
	public void replyInsert(int pno,BoardVO vo)
	{
		try
		{
			getConnection();
			conn.setAutoCommit(false);
			// SQL 문장 여러 개 있는 경우 => 트랜잭션
			// 상위 게시물의 group_id, group_step, group_tab
	...
	public boolean boardDelete(int no,String pwd)
	{
		...
		try
		{
			...
			// depth
			// 삭제 여부 확인 depth=0일 때 삭제 대상(아래 댓글 없음)
			conn.commit();
		}catch(Exception ex)
		{
			ex.printStackTrace();
